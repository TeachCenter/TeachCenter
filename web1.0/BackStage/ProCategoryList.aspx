	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><asp:LinkButton ID="lbtDeleteMore" runat="server" class="btn btn-danger radius" OnClick="lbtDeleteMore_Click" OnClientClick="return confirm('确定删除?')"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</asp:LinkButton> <a class="btn btn-primary radius" data-title="添加项目类型" data-href="ProjectAdd.aspx" onclick="Hui_admin_tab(this)" href="ProCategoryAdd.aspx"><i class="Hui-iconfont">&#xe600;</i> 添加项目类型</a></span> <span class="r">共有数据：<strong><asp:Literal ID="ltSum" runat="server"></asp:Literal></strong> 条</span> </div>
        <div class="mt-20">
		    <table class="table table-border table-bordered table-bg table-hover table-responsive">
			    ...
			    <tbody>
                    <asp:Repeater ID="rptCategory" runat="server" OnItemCommand="rptCategory_ItemCommand" ItemType="ProjectCategory">
                        <ItemTemplate>
				            <tr class="text-c">
					            <td><input type="checkbox" runat="server" id="isDelete"></td>					
					            <td class="text-l"><u style="cursor:pointer" class="text-primary")" title="查看"><a href="ProCategoryContent.aspx?id=<%# Item.id%>"><%# Item.name%></a></u></td>	
                                <td><%# UeditorHelper.NoHTML(Item.project_content) %><input type="hidden" runat="server" id="id" value=<%# Item.id%> /></td>
					            <td><%# AdminHelper.judgeStage(Convert.ToInt32(Item.stage))%></td>
					            <td><%# Item.end_time %></td>
                                <td class="f-14 td-manage"><asp:LinkButton ID="lbtDelete" runat="server" CssClass="ml-5" CommandName="Delete" CommandArgument='<%# Item.id%>' OnClientClick="return confirm('确定删除?')">删除<i class="Hui-iconfont">&#xe6e2;</i></asp:LinkButton></td>
				            </tr>
                        </ItemTemplate>
                    </asp:Repeater>

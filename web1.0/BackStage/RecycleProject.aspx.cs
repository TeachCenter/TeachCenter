...
public partial class BackStage_RecycleProject : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    // 绑定项目列表
                    var project = from it in db.ProjectInfo where it.is_deleted == 1 orderby it.submit_time descending select it;
                    ...
                    // 绑定项目分类下拉框
                    var category = from it in db.ProjectCategory where it.is_deleted == 1 select it;
    ...
    protected void DropDownList_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        using (var db = new TeachingCenterEntities())
        {
            var project = from it in db.ProjectInfo where it.category == category && it.is_deleted == 1 orderby it.submit_time descending select it;
            if (category == 0)
                project = from it in db.ProjectInfo where it.is_deleted == 1 orderby it.submit_time descending select it;
            ltSum.Text = project.Count().ToString();
            currentPage.Text = "1";
            TotalPage.Text = Math.Ceiling(project.Count() / 10.0).ToString();
            rptProject.DataSource = project.ToList().Take(10);
    ...
    protected void lbtSearch_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            using (var db = new TeachingCenterEntities())
            {
                ...
                {
                    project = from it in db.ProjectInfo where it.name.IndexOf(project_name) >= 0 && it.is_deleted == 1 orderby it.submit_time descending select it;
                }
                if (category != 0)
                {
                    project = from it in db.ProjectInfo where it.category == category && it.is_deleted == 1 orderby it.submit_time descending select it;
                    if (project_name != "")
                        project = from it in db.ProjectInfo where it.category == category && it.name.IndexOf(project_name) >= 0 && it.is_deleted == 1 orderby it.submit_time descending select it;
                }
                ...
                {
                    Response.Write("<script>alert('搜索无结果！');location.href='RecycleProject.aspx';</script>");
                }
            }
        }
    }

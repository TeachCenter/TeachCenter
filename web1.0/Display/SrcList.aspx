        ...
        <span class="leftLine"></span>
			<div class="center clearfix">
				<div class="left-content">
					<a class="left-content-nav">教学资源</a>
    ...
    <script>
		...
		$.ajax({ 
          ...
          success: function(data) {
          	...
          	{
          	    if (type == data[0][i].SourceCategory_id) {
          	        $("<a>").addClass("left-content-nav-content-select").attr("href", (data[0][i].SourceCategory_href )).text(data[0][i].SourceCategory_name).appendTo($(".left-content"));
          	    }
          	    else {

          	        $("<a>").addClass("left-content-nav-content").attr("href", (data[0][i].SourceCategory_href  )).text(data[0][i].SourceCategory_name).appendTo($(".left-content"));
          	...
          	{
          	    var content = $("<a>").addClass("passage").addClass("clearfix").attr("href", ( data[2][i].Source_href + data[2][i].Source_id)).appendTo($(".passage-content"));
          		$("<h2>").text(data[2][i].Source_title).appendTo(content);
          		var pContent=$("<div>").appendTo(content);
          		$("<p>").text(data[2][i].Source_summary).appendTo(pContent);
          		$("<h3>").text("发布人："+data[2][i].Source_publisher+" 发布时间："+data[2][i].Source_time+" 浏览次数："+data[2][i].Source_views).appendTo(content);
          		$("<a>").attr("href", (data[2][i].Source_href + data[2][i].Source_id)).text("查看文章").appendTo(content);
